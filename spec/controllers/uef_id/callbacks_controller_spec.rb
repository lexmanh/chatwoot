...
RSpec.describe 'UEF::CallbacksController', type: :request do
  let(:account) { create(:account) }
  let(:code) { SecureRandom.hex(10) }
  let(:email) { Faker::Internet.email }
  let(:state) { account.to_sgid(expires_in: 15.minutes).to_s }

  describe 'GET /uef_id/callback' do
    ...
    it 'creates inboxes if authentication is successful' do
      stub_request(:post, 'https://accounts.uef.edu.vn/realms/university/protocol/openid-connect/token')
        .with(body: { 'code' => code, 'grant_type' => 'authorization_code',
                      'redirect_uri' => "#{ENV.fetch('FRONTEND_URL', 'http://localhost:3000')}/uef_id/callback" })
        .to_return(status: 200, body: response_body_success.to_json, headers: { 'Content-Type' => 'application/json' })

      get uef_id_callback_url, params: { code: code, state: state }
    ...
    it 'creates inboxes with fallback_name when account name is not present in id_token' do
      stub_request(:post, 'https://sso.uef.edu.vn/realms/university/protocol/openid-connect/token')
        .with(body: { 'code' => code, 'grant_type' => 'authorization_code',
                      'redirect_uri' => "#{ENV.fetch('FRONTEND_URL', 'http://localhost:3000')}/uef_id/callback" })
    ...
    it 'redirects to uef_id app in case of error' do
      stub_request(:post, 'https://sso.uef.edu.vn/realms/university/protocol/openid-connect/token')
        .with(body: { 'code' => code, 'grant_type' => 'authorization_code',
                      'redirect_uri' => "#{ENV.fetch('FRONTEND_URL', 'http://localhost:3000')}/uef_id/callback" })
